...
// E-Pao keyboard → Unicode Meitei Mayek (mirrors convert_to_unicode.py)
const M = {
...
const POSMAP = { n: "noun", "n+": "noun", v: "verb", v_plus: "verb", adv: "adverb", adj: "adjective", interj: "interjection", prep: "preposition", conj: "conjunction", pron: "pronoun", num: "numeral" };
const HW = /^[a-z][a-z.' -]{0,39}$/;
...
for (const file of files) {
  ...
  // per-IPA split: each entry has one /ipa/ block
  const ipaRe = /([a-z][a-z.' -]{0,40})\//g;
  let mm, last = 0, parts = [];
  while ((mm = ipaRe.exec(text)) !== null) {
    parts.push({ pre: text.slice(last, mm.index), ipa: mm[1].trim() });
    last = mm.index + mm[0].length;
  }
  if (!parts.length) continue;
  for (let i = 0; i < parts.length; i++) {
    const { pre, ipa } = parts[i];
    const hw = pre.match(/[^,;:()\[\]~][a-z.' ]*$/);
    const word = hw ? hw[0].trim() : "";
    if (!HW.test(word) || word.startsWith("and ") || word.startsWith("of ") || word.split(" ").length > 2) continue;
    // gloss = ipa tail-up to next entry's headword (heuristic: strip trailing Morph)
    let gloss = "";
    const next = i + 1 < parts.length ? parts[i + 1].pre : "";
    gloss = next;
    gloss = gloss.replace(/Morph\s*:.*$/s, "").replace(/^\s*[,.;]\s*/, "").trim();
    const posm = gloss.match(/^(n|n\+|v|v\+|adv|adv\+|adj|adj\+|interj|prep|conj|pron|num)?\.?\s*/);
    const pos = posm ? (POSMAP[posm[1] || ""] || posm[1] || "") : "";
    gloss = clean(posm ? gloss.slice(posm[0].length) : gloss).replace(/^\d+\s*/, "");
    const mayek = toMayek(word);
    if (gloss.length > 2 && mayek) entries.push({ word, mayek, ipa, pos, gloss, page: n });
  }
}
...
(async () => {
  ...
  console.log(`Unique headwords: ${new Set(uniq.map(e=>e.word.toLowerCase())).size} | New vs DB: ${toInsert.length}`);

  let ins = 0;
  if (process.argv.includes("--dry")) {
    await c.end();
    console.log("DRY MODE: no rows written. Re-run without --dry to import.");
    return;
  }
  for (const e of toInsert) {
    const sl = slugify(e.word);
    const dup = await c.query("SELECT id FROM words WHERE slug = ? LIMIT 1", [sl]);
    const finalSlug = dup.length ? sl + "-" + ins : sl;
    ...
    await c.query(`INSERT INTO word_senses (word_id, wordtype, wordtype_raw, definition, meaning_eng_man, meaning_mm, meaning_mm_unicode, antonyms, synonyms, status, submitted_by, reviewed_by, reviewed_at)
      VALUES (?,?,?,?,?,?,?,'','','approved',1,1,NOW())`, [r.insertId, e.pos || "unknown", e.pos || "", e.gloss, e.gloss, e.word, e.mayek]);
    ins++;
    if (ins % 500 === 0) console.log(`  ...${ins}`);
  }
